...
export function D3Animation({
  ...
  const generateD3Visualization = useCallback((step: AnimationStep, algorithmId: string) => {
    ...
    if (data.array && Array.isArray(data.array)) {
      // Check if it's a two-pointer problem (target exists)
      if (data.target !== undefined || data.hashMap) {
        return generateTwoSumD3(step)
      }
      // Check if it's a container/area problem (left/right pointers)
    ...
    if (data.string || data.original || data.text || (typeof data.value === 'string' && data.value.length > 1)) {
      return generateStringManipulationD3(step)
    }

    // Detect number/math operations
    if (typeof data.value === 'number' || data.digit !== undefined || data.reversed !== undefined) {
