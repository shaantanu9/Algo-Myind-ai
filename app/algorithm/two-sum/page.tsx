...
const ALGORITHM_DATA: AlgorithmData = {
  problemId: 1,
  title: "Two Sum",
  description: "A array algorithm that uses iterative approach to solve the problem with O(n) time complexity.",
  difficulty: "Easy",
  category: "Array",
  timeComplexity: "O(n)",
  spaceComplexity: "O(1)",
  ...
  problemStatement: "Given an array of integers, perform operations to solve the problem efficiently.",
  realWorldUse: "Data processing, scientific computing, game development",
  constraints: [
  "1 <= nums.length <= 10^4",
  "-10^9 <= nums[i] <= 10^9"
],
  solution: {
  "javascript": "// Test JavaScript algorithm file for upload testing\n// Problem: Two Sum\n// Difficulty: Easy\n\n/**\n * @param {number[]} nums\n * @param {number} target\n * @return {number[]}\n */\nvar twoSum = function(nums, target) {\n    const map = new Map();\n\n    for (let i = 0; i < nums.length; i++) {\n        const complement = target - nums[i];\n\n        if (map.has(complement)) {\n            return [map.get(complement), i];\n        }\n\n        map.set(nums[i], i);\n    }\n\n    return [];\n};\n\n// Example usage:\n// twoSum([2, 7, 11, 15], 9) should return [0, 1]\n// twoSum([3, 2, 4], 6) should return [1, 2]\n",
  ...
  "visualizationHints": "Show array operations and data flow"
}
  },
  metadata: {
    tags: [
...
export default function TwosumPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const localStorageAlgorithm = LocalStorageManager.loadAlgorithm("two-sum")
      ...
      LocalStorageManager.markAlgorithmAsUsed("two-sum")
    }
  }, [])
                    ...
                    <>
                      {selectedVisualization === 'mermaid' && (
                        <MermaidAnimation
                          ...
                          algorithmId={algorithmData.id || "two-sum"}
                        />
                      )}
